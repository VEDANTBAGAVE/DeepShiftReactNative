...
const SubmittedLogsScreen: React.FC = () => {
  ...
  const shiftLogs = useMemo<ShiftLog[]>(() =>
    shifts.map(s => ({
      id: s.id,
      date: new Date(s.shift_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }),
      dayOfWeek: new Date(s.shift_date).toLocaleDateString('en-IN', { weekday: 'long' }),
      shiftType: `${s.shift_type.charAt(0).toUpperCase() + s.shift_type.slice(1)} Shift`,
      status: s.status === 'submitted' ? 'submitted' : s.status === 'approved' ? 'approved' : s.status,
      submittedAt: s.submitted_at
        ? new Date(s.submitted_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        : '—',
      remarks: s.handover_notes ?? '',
    })),
  [shifts]);
          ...
          filteredLogs.map(log => (
            ...
            >
              ...
              <View style={styles.shiftInfo}>
                ...
                <Text style={styles.shiftTime}>� {log.submittedAt}</Text>
              </View>

              {/* Remarks */}
              {log.remarks ? (
                <View style={styles.submissionInfo}>
                  <Text style={styles.submissionText}>💬 {log.remarks}</Text>
                </View>
              ) : null}
              ...
              >
                ...
                <View style={styles.modalSection}>
                  ...
                  <View style={styles.modalInfoRow}>
                    ...
                    <Text style={styles.modalValue}>{selectedLog.shiftType}</Text>
                  </View>
                  <View style={styles.modalInfoRow}>
                    <Text style={styles.modalLabel}>Submitted At</Text>
                    <Text style={styles.modalValue}>{selectedLog.submittedAt}</Text>
                  </View>
                  <View style={styles.modalInfoRow}>
                ...
                {selectedLog.remarks ? (
                  <View style={styles.modalSection}>
                    ...
                    <View style={styles.remarksBox}>
                      <Text style={styles.remarksText}>{selectedLog.remarks}</Text>
                    </View>
                  </View>
                ) : null}
